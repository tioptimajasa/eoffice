            ...
            <div class="col-md-12">
                <div class="card card-success">
                    <div class="card-header">
                        <h3 class="card-title">Create Memo  {{ $action }}</h3>
                    </div>
                    @if ($action === 'create')
                    <form action="{{ route('memo.store') }}" method="POST" enctype="multipart/form-data" id="form_memo">
                        @php
                            $kepada ="";
                            $dari = "";
                            $isi = "";
                    ...
                    @elseif ($action === 'edit')
                    <form action="{{ route('memo.update', $data_memo->id) }}" method="POST" enctype="multipart/form-data" id="form_memo">
                    @else
                        ...
                        <div class="card-body">
                            <label for="exampleInputEmail1">Kepada</label>
                                <select id="kepada_id" name="kepada_id" required="required" class="form-control">
                                    <option></option>
                                    @foreach($memo_kepada as $data_struktur)
                            ...
                            <label for="exampleInputEmail1">Dari</label>
                                <select id="dari_id" name="dari_id" required="required" class="form-control">
                                    <option></option>
                                    @foreach ($memo_dari as $data_struktur)
                                        <option value="{{$data_struktur->id}}">{{$data_struktur->jabatan}} {{$data_struktur->nama_organisasi}}</option>
                                    @endforeach
                                </select>
                                ...
                                <select id="pemeriksa" name="pemeriksa[]" multiple required="required" class="form-control">
                                    <option></option>
                                    @foreach ($memo_pemeriksa as $data_struktur_pemeriksa)
                                ...
                                <input type="file" class="form-control" name="lampiran_memo" required>
